...
module "yaml-to-standard" {
  source           = "git::https://github.com/<org-name>/terraform-azure-storage-account//modules//yaml-to-standard?ref=[git-tag]"
  yaml_config_path = "../../data/standard"
}
```
3. **Use the output to create new Storage Accounts**

The module will output a map of Storage Account configurations that can be used with the [terraform-azure-storage-account](https://github.com/<org-name>/terraform-azure-storage-account) module. There are many options available and you may wish to override some of the defaults. Below is an example of how to use the output from the yaml-to-standard module to create Storage Accounts and additioanlly role assignments at the container scope:
```hcl
module "storage" {
  source   = "git::https://github.com/<org-name>/terraform-azure-storage-account//modules//standard?ref=[git-tag]"
...
module "yaml-to-standard-private-only" {
  source           = "git::https://github.com/<org-name>/terraform-azure-storage-account//modules//yaml-standard-private-only?ref=[git-tag]"
...
```hcl
module "storage" {
  source   = "git::https://github.com/<org-name>/terraform-azure-storage-account//modules//standard-private-only?ref=[git-tag]"
...
module "hardcoded_storage" {
  source = "git::https://github.com/<org-name>/terraform-azure-storage-account//standard?ref=[git-tag]"
