...
module "storage" {
  ...
  providers = {
    azurerm              = azurerm
    azurerm.pe-dns-infra = azurerm.racwa-connectivity
  }
...
Note: The `azurerm.racwa-connectivity` provider is required to manage the private DNS zone for the storage account private endpoints. Ensure that you have the necessary permissions and configurations in place for this provider.

```hcl
provider "azurerm" {
  alias               = "racwa-connectivity"
...
module "hardcoded_storage" {
  source = "git::https://github.com/racwa/terraform-azure-storage-account//standard?ref=[git-tag]"
