...
resource "azurerm_role_assignment" "group" {
  ...
  principal_id         = data.azuread_group.lookup[each.value.principal].object_id
}
...
resource "azurerm_role_assignment" "user" {
  ...
  role_definition_name = each.value.role
  principal_id         = data.azuread_user.lookup[each.value.principal].object_id
}

resource "azurerm_role_assignment" "sp" {
  for_each = var.sp_assignments
  ...
  role_definition_name = each.value.role
  principal_id         = data.azuread_service_principal.lookup[each.value.principal].object_id
}